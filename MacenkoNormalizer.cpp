...
void ColourStainNormalization::MacenkoNormalizer::transform(cv::Mat source, cv::Mat &output)
{
    ...
    computeStainMatrix(stdoutput, _stainMatrix);
    computeConcentrationMatrix(stdoutput, _stainMatrix, _concentrationMatrix);
    double *data = _concentrationMatrix.col(0).data();
    vector<double> columnVectors(data, data + _concentrationMatrix.rows());
    std::sort(columnVectors.begin(), columnVectors.end());
    double maxC[2];
    ...
    data = _concentrationMatrix.col(1).data();
    columnVectors.erase(columnVectors.begin(), columnVectors.end());
    vector<double> columnVectors2(data, data + _concentrationMatrix.rows());
    std::sort(columnVectors2.begin(), columnVectors2.end());
